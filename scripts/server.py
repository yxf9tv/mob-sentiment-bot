...
@app.get("/api/backtest/pregame")
def get_pregame_backtest():
    """Return pre-game backtest results (pre-game vs full-data comparison)."""
    path = DATA_DIR / "backtest_pregame_results.json"
    if not path.exists():
        return {"error": "no pre-game backtest data", "hint": "Run scripts/backtest_pre_game.py first"}
    with open(path) as f:
        return json.load(f)


@app.get("/api/pregame/snapshots")
def get_pregame_snapshots():
    """Return accumulated pre-game snapshots (latest N)."""
    path = DATA_DIR / "pregame_snapshots.jsonl"
    if not path.exists():
    ...
    # Return latest 100
    snapshots.reverse()
    return {"snapshots": snapshots[:100], "count": len(snapshots)}


@app.get("/api/triggers")
def get_triggers():
    """Return all logged threshold crossing events."""
    path = DATA_DIR / "trigger_events.jsonl"
    if not path.exists():
    ...
    for e in events:
        ...
        if t not in by_threshold:
            by_threshold[t] = {"count": 0, "description": e.get("description", ""), "min_volume": e.get("min_volume", 0)}
        by_threshold[t]["count"] += 1
    return {"events": events, "count": len(events), "by_threshold": by_threshold}
...
def trigger_poll():
    """Run the poll script as a subprocess (non-blocking)."""
