...
function computeConfidence(m) {
  let mt = m.market_type;
  let vol = m.total_weighted_volume || 0;
  let tr = m.unique_traders || 0;
  // Backtest-derived expected accuracy
  if (mt === "moneyline" && vol >= 1071) return { acc: 0.744, label: "74%", action:"BET", cls:"text-green" };
  if (mt === "spread" && vol >= 107) return { acc: 0.731, label: "73%", action:"BET", cls:"text-green" };
  if (mt === "moneyline" && vol >= 107) return { acc: 0.640, label: "64%", action:"BET", cls:"text-green" };
  if (mt === "total") return { acc: 0.573, label: "57%\u2192", action:"FADE", cls:"text-yellow" };
  return null;
}

function confidenceBadge(m) {
  let c = computeConfidence(m);
  if (!c) return "";
  return `<span class="tag tag-${c.action==="FADE"?"flag":"clean"}">${c.label}</span>`;
}

function confidenceColor(acc) {
  return acc >= 0.6 ? "var(--green)" : acc >= 0.5 ? "var(--yellow)" : "var(--red)";
}
    ...
    if (bets.length) {
      ...
      for (let r of bets) {
        html+=`<div class="rec-row">
          ...
          <span class="rec-edge">${r.confidence==="high"?"\uD83D\uDFE2":"\uD83D\uDFE1"} ${(r.expected_accuracy*100).toFixed(0)}%</span>
        </div>`;
      }
      html+=`</div>`;
    }
...
function renderLive() {
  ...
  let edgeFilter = document.getElementById("liveEdge").checked;
  ...
  // Edge filter: keep only +EV signals
  if (edgeFilter) {
    let evCount = allMarkets.length;
    allMarkets = allMarkets.filter(m => computeConfidence(m) !== null);
    // Sort by confidence descending
    allMarkets.sort((a,b) => {
      let ea = computeConfidence(a), eb = computeConfidence(b);
      return (eb ? eb.acc : 0) - (ea ? ea.acc : 0);
    });
  } else {
  ...
  if (allMarkets.length) {
    summaryHtml = `<div class="summary-section">
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Market</th><th>Date</th><th>Conf</th><th>Type</th><th>Lean</th><th>Frac</th><th>Conviction</th><th>Depth</th><th>Traders</th><th>Trades</th></tr></thead>
        ${allMarkets.map(m => {
          let gd=m.game_date||"";
  ...
  for (let g of filteredGames) {
    ...
    for (let m of markets) {
      ...
      let conf = computeConfidence(m);
      let edgeHtml = conf ? confidenceBadge(m) : "";
      if (hasData) {
        html+=`<div class="signal-card" style="${edgeFilter && !conf ? 'display:none' : ''}">
          <div class="sc-header">${marketTypeTag(m.market_type)} ${edgeHtml} <span class="sc-slug" title="${m.slug}">${fmtSlugged}</span></div>
          <div class="sc-body">
            ...
            ${conf ? `<div class="sc-conv"><span class="sc-label">Confidence</span> <span style="color:${confidenceColor(conf.acc)};font-weight:600;font-size:15px">${conf.label}</span></div>` : ""}
...
async function triggerPoll() {
  ...
  if (result && result.status==="ok") {
    ...
    // Retry reloading data every 5s until poll finishes
    let waited=0;
...
function renderPregame() {
  api("/api/backtest/pregame").then(data=>{
    if (!data || data.error) {
      document.getElementById("pgHeader").innerHTML=`
        <div style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:24px;margin-bottom:16px">
          <p style="color:var(--text2);margin-bottom:8px">No pre-game backtest results yet.</p>
          <p>Run <code style="background:var(--bg3);padding:2px 6px;border-radius:4px">python3 scripts/backtest_pre_game.py</code> from the project root to generate them.</p>
        </div>`;
      return;
    }

    let s = data.summary || {};
    let header = document.getElementById("pgHeader");
    header.innerHTML = `
      <div style="margin-bottom:12px;color:var(--text2);font-size:13px">
        Data range: ${data.data_range.start} to ${data.data_range.end}
        &middot; Game start heuristic: ${data.game_start_heuristic_utc} UTC
        &middot; ${s.total_markets} markets, ${s.results} with trade data
      </div>
      <div class="stats-row">
        <div class="stat-card"><div class="num">${s.resolved_pre_game}</div><div class="label">Pre-Game Resolved</div></div>
        <div class="stat-card ${(s.pre_game_accuracy||0)>=0.5?'text-green':'text-red'}">
          <div class="num">${fmtPct(s.pre_game_accuracy||0)}</div><div class="label">Pre-Game Accuracy</div></div>
        <div class="stat-card"><div class="num">${s.resolved_full_data}</div><div class="label">Full-Data Resolved</div></div>
        <div class="stat-card ${(s.full_data_accuracy||0)>=0.5?'text-green':'text-red'}">
          <div class="num">${fmtPct(s.full_data_accuracy||0)}</div><div class="label">Full-Data Accuracy</div></div>
        <div class="stat-card"><div class="num">${s.same_prediction_pct||0}%</div><div class="label">Same Prediction</div></div>
        <div class="stat-card"><div class="num">${s.change_count||0}</div><div class="label">Predictions Changed</div></div>
      </div>`;

    // Comparison table
    let byType = data.by_market_type || {};
    let typeRows = Object.entries(byType).sort(([,a],[,b])=>b.total-a.total).map(([mt,d])=>{
      let pre = d.pre_game || {}, full = d.full_data || {};
      let preAcc = pre.resolved ? fmtPct(pre.accuracy) : '-';
      let fullAcc = full.resolved ? fmtPct(full.accuracy) : '-';
      let preCls = pre.accuracy >= 0.5 ? 'text-green' : 'text-red';
      let fullCls = full.accuracy >= 0.5 ? 'text-green' : 'text-red';
      return `<tr>
        <td>${marketTypeTag(mt)} ${mt}</td>
        <td class="text-right">${d.total}</td>
        <td class="text-right">${pre.resolved}</td>
        <td class="text-right ${preCls}">${preAcc}</td>
        <td class="text-right">${full.resolved}</td>
        <td class="text-right ${fullCls}">${fullAcc}</td>
        <td class="text-right">${d.predictions_changed||0}</td>
      </tr>`;
    }).join("");

    document.getElementById("pgComparison").innerHTML = `
      <div class="bt-card">
        <h3 class="bt-heading">Pre-Game vs Full-Data Accuracy by Market Type</h3>
        <p style="font-size:12px;color:var(--text2);margin-bottom:12px">
          Pre-game filters trades to only those before ${data.game_start_heuristic_utc} UTC on game date.
          Full-data includes all trades. "Changed" = markets where the top outcome flipped after adding post-start trades.
        </p>
        <div class="table-wrap"><table>
          <thead><tr><th>Market Type</th><th>Total</th><th>Pre-Game Resolved</th><th>Pre-Game Acc</th><th>Full Resolved</th><th>Full Acc</th><th>Changed</th></tr></thead>
          ${typeRows}
        </table></div>
      </div>`;
    ...
    let detailRows = results.filter(r => r.actual_outcome)
      .sort((a,b) => Math.abs((b.pre_game||{}).conviction||0) - Math.abs((a.pre_game||{}).conviction||0))
      .map(r => {
        let pre = r.pre_game || {}, full = r.full_data || {};
        let preOutcome = pre.top_outcome || '-';
        let fullOutcome = full.top_outcome || '-';
        let actual = r.actual_outcome || '?';
        let preIcon = r.pre_game_correct === true ? '\u2713' : r.pre_game_correct === false ? '\u2717' : '-';
        let fullIcon = r.full_data_correct === true ? '\u2713' : r.full_data_correct === false ? '\u2717' : '-';
        let preCls = r.pre_game_correct === true ? 'text-green' : r.pre_game_correct === false ? 'text-red' : '';
        let fullCls = r.full_data_correct === true ? 'text-green' : r.full_data_correct === false ? 'text-red' : '';
        let changed = (pre.top_outcome && full.top_outcome && pre.top_outcome !== full.top_outcome) ? 'Yes' : '';
        return `<tr>
          <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis" title="${r.slug}">${fmtSlug(r.slug)}</td>
          <td>${r.game_date||'-'}</td>
          <td>${marketTypeTag(r.market_type)}</td>
          <td>${preOutcome}</td>
          <td class="${preCls}" style="font-size:16px;text-align:center">${preIcon}</td>
          <td>${fullOutcome}</td>
          <td class="${fullCls}" style="font-size:16px;text-align:center">${fullIcon}</td>
          <td>${actual}</td>
          <td style="color:${changed?'var(--attention)':'var(--text2)'}">${changed||'-'}</td>
        </tr>`;
      }).join("");

    document.getElementById("pgDetails").innerHTML = `
      <div class="bt-card">
        <h3 class="bt-heading">Individual Market Comparison <span class="date-count">${results.filter(r=>r.actual_outcome).length} resolved</span></h3>
        <div class="table-wrap" style="max-height:600px;overflow-y:auto"><table>
          <thead><tr><th>Market</th><th>Date</th><th>Type</th><th>Pre-Game Pick</th><th>Pre Result</th><th>Full Pick</th><th>Full Result</th><th>Actual</th><th>Changed</th></tr></thead>
          ${detailRows||'<tr><td colspan="9" style="text-align:center;color:var(--text2)">No resolved results</td></tr>'}
        </table></div>
      </div>`;
  });

  // Load trigger events
  api("/api/triggers").then(trig => {
    if (trig && trig.count > 0) {
      let byThresh = trig.by_threshold || {};
      let threshHtml = Object.entries(byThresh).sort((a,b) => b[1].count - a[1].count).map(([label, info]) => {
        return `<span style="background:var(--bg3);padding:2px 8px;border-radius:4px;font-size:12px">
          <strong>${label}</strong> ${info.count}x &mdash; ${info.description}
        </span>`;
      }).join(" ");
      let recentHtml = trig.events.slice(-10).reverse().map(e => {
        let ts = e.crossed_at.slice(0, 19).replace("T", " ");
        return `<tr>
          <td style="white-space:nowrap;font-size:12px">${ts}</td>
          <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis" title="${e.slug}">${fmtSlug(e.slug)}</td>
          <td>${marketTypeTag(e.market_type)}</td>
          <td style="font-weight:500">${e.threshold}</td>
          <td class="text-right">${fmtDollar(e.volume)}</td>
          <td>${e.top_outcome}</td>
          <td>${e.conviction ? fmtPct(e.conviction) : '-'}</td>
        </tr>`;
      }).join("");

      document.getElementById("pgDetails").innerHTML += `
        <div class="bt-card" style="margin-top:16px">
          <h3 class="bt-heading">Trigger Events <span class="date-count">${trig.count} crossings</span></h3>
          <p style="font-size:12px;color:var(--text2);margin-bottom:12px">
            First time a market's total_weighted_volume crosses an edge threshold.
          </p>
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">${threshHtml}</div>
          <div class="table-wrap" style="max-height:500px;overflow-y:auto"><table>
            <thead><tr><th>Time</th><th>Market</th><th>Type</th><th>Threshold</th><th>Volume</th><th>Pick</th><th>Conviction</th></tr></thead>
            ${recentHtml||'<tr><td colspan="7" style="text-align:center;color:var(--text2)">No events</td></tr>'}
          </table></div>
        </div>`;
    }
  });

  // Load snapshot count from forward-looking system
  api("/api/pregame/snapshots").then(snap => {
    if (snap && snap.count) {
      let hdr = document.getElementById("pgHeader");
      let existing = hdr.innerHTML;
      hdr.innerHTML = existing + `
        <div style="margin-top:12px;padding:12px 16px;background:var(--bg2);border:1px solid var(--border);border-radius:8px">
          <span style="color:var(--text2)">Forward-looking snapshots accumulated:</span>
          <strong>${snap.count}</strong> poll cycles,
          <strong>${snap.snapshots ? snap.snapshots[0]?.game_count||0 : 0}</strong> games in latest snapshot
        </div>`;
    }
  });
}
